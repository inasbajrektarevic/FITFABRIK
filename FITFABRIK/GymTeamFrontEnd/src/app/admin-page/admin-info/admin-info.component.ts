...
export class AdminInfoComponent implements OnInit {
  ...
  editData() {
    ...
    this.buttonName = this.enableEdit ? 'Save' : 'Edit data';
    this.counter++;
    if (this.counter === 3) {
      this.saveChanges();
    }
  ...
  saveChanges() {
    ...
    if (!this.enableEdit && !!this.changeData) {
      this.httpClient.put(`${url}${this.userId}`, this.editUser).subscribe((res) => {
        if (!!res) {
  ...
  async updateProfileImage(): Promise<any> {
    ...
    try {
      const res = await this.httpClient.put(url, formData, { headers }).toPromise();
